...
module.exports = class Hermione extends BaseHermione {
    ...
    async run(testPaths, {browsers, sets, grep, updateRefs, reporters, inspectMode} = {}) {
        ...
        await this._init();
        await this._run(await this._readTests(testPaths, {browsers, sets, grep}));
    ...
    async _run(testCollection) {
        try {
            await this.emitAndWait(RunnerEvents.RUNNER_START, this._runner);
            await this._runner.run(testCollection);
        } finally {
            await this.emitAndWait(RunnerEvents.RUNNER_END, this._stats.getResult()).catch(logger.warn);
        }
    }
